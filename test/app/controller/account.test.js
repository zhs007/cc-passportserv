...
describe('test/app/controller/account.test.js', async () => {
  ...
  it('should POST /api/account/register', async () => {
    ...
    const pair = [
      [{ email: 'a@b.c' }, { code: ERR_VALIDATE_PARAMS, errors: [{ message: 'required', field: 'username', code: 'missing_field' }, { message: 'required', field: 'password', code: 'missing_field' }] }, 400 ],
      [{ email: 'a@b', username: 'abcd', password: '12345678' }, { code: ERR_EMAIL_FORMAT }, 400 ],
      [{ email: 'a@b.c', username: 'a01234567890123456789', password: '12345678' }, { code: ERR_USERNAME_FORMAT }, 400 ],
      [{ email: 'a@b.c', username: 'abcde', password: '1234567' }, { code: ERR_PASSWORD_FORMAT }, 400 ],
  ...
  it('should POST /api/account/login', async () => {
    ...
    const pair = [
      [{ email: 'a@b.c' }, { code: ERR_VALIDATE_PARAMS, errors: [{ message: 'required', field: 'password', code: 'missing_field' }] }, 400 ],
      [{ email: 'a@b', password: '12345678' }, { code: ERR_EMAIL_FORMAT }, 400 ],
      [{ email: 'a@b.c', password: '1234567' }, { code: ERR_PASSWORD_FORMAT }, 400 ],
  ...
  it('should GET /api/account/checkemail', async () => {
    app.mockCsrf();

    const pair = [
      [{}, { code: ERR_VALIDATE_PARAMS, errors: [{ message: 'required', field: 'email', code: 'missing_field' }] }, 400 ],
      [{ email: 'a@b' }, { code: ERR_EMAIL_FORMAT }, 400 ],
    ...
    for (let i = 0; i < pair.length; ++i) {
      if (pair[i][0].email) {
        const result = await app.httpRequest()
          .get('/api/account/checkemail?email=' + pair[i][0].email)
          .expect(pair[i][2]);
  ...
  it('should GET /api/account/checkusername', async () => {
    app.mockCsrf();

    const pair = [
      [{}, { code: ERR_VALIDATE_PARAMS, errors: [{ message: 'required', field: 'username', code: 'missing_field' }] }, 400 ],
      [{ username: 'ab' }, { code: ERR_USERNAME_FORMAT }, 400 ],
      [{ username: 'user001' }, { code: ERR_DUP_USERNAME }, 200 ],
      [{ username: 'user003' }, { code: OK }, 200 ],
    ];

    for (let i = 0; i < pair.length; ++i) {
      if (pair[i][0].username) {
        const result = await app.httpRequest()
          .get('/api/account/checkusername?username=' + pair[i][0].username)
          .expect(pair[i][2]);
